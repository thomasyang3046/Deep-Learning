...
class DQN:
    ...
    def select_action(self, state, epsilon, action_space):
        ...
        with torch.no_grad():
            ...
            Qvalue = self._behavior_net(state)
            _, best_action = torch.max(Qvalue, 1)
...
def train(args, agent, writer):
    ...
    action_space = env.action_space # 四個動作 0~3
    total_steps, epsilon = 0, 1
    ewma_reward = 0
    #print(action_space.sample())
    for episode in range(0,args.episode):
        total_reward = 0
        state = env.reset()#初始化第一個狀態
        #print(state.shape)
        state, reward, done, _ = env.step(1) # fire first !!! 每個狀態都是一張圖
        for t in itertools.count(start=1):
            ...
            if total_steps % args.eval_freq == 0 and total_steps!=0 :
                ...
                agent.save(args.model + "dqnbreakout2_" + str(total_steps) + ".pt")
...
def main():
    ...
    parser.add_argument('--logdir', default='log/breakout2')
    # train
    parser.add_argument('--warmup', default=20000, type=int)
    parser.add_argument('--episode', default=28500, type=int)
    ...
    parser.add_argument('-tmp', '--test_model_path', default='ckpt/dqnbreakout2_4500000.pt')
