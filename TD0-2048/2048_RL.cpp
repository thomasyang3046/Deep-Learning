...
public:
	 ...
	 * get a 16-bit row
	 */
	int  fetch(int i) const { return ((raw >> (i << 4)) & 0xffff); } //取第i個row
	/**
	 * set a 16-bit row
	 */
	void place(int i, int r) { raw = (raw & ~(0xffffULL << (i << 4))) | (uint64_t(r & 0xffff) << (i << 4)); } //設定第i個row
	/**
	 * get a 4-bit tile
	 */
	int  at(int i) const { return (raw >> (i << 2)) & 0x0f; } //取某個block
	/**
	 * set a 4-bit tile
	 */
	void set(int i, int t) { raw = (raw & ~(0x0fULL << (i << 2))) | (uint64_t(t & 0x0f) << (i << 2)); } //設定某個block
...
public:
	pattern(const std::vector<int>& p, int iso = 8) : feature(1 << (p.size() * 4)), iso_last(iso) {
		 ...
		 */
		//利用所有 pattern的總值來代表當board的期望值 ，所以會用8個 isomorphic pattern的weight值加總得到這個pattern的值。
		for (int i = 0; i < 8; i++) { //8 isomorphisms
...
public:
	 ...
	 * estimate the value of a given board
	 */
	virtual float estimate(const board& b) const { //value function (V(s))
		// TODO
		float value = 0.f;
		for (int i = 0; i < iso_last; i++)
			value += operator[](indexof(isomorphic[i], b)); //加總每個index對應的weight
		return value;
	}
	...
	virtual float update(const board& b, float u) { 
		// TODO
		//Look up the weight of isomorphic in table, then update its value.
		float value = 0.f;
		for (int i = 0; i < iso_last; i++) 
		{
			operator[](indexof(isomorphic[i], b)) += (u / iso_last); 
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {//取得isomorphic pattern在當下board的一個值， 並把這個值當成一個index
		// TODO
		size_t index = 0;
		for (int i = 0; i < patt.size(); i++)
			index = index | b.at(patt[i]) << (4 * i);
		return index;
	}
...
public:
	...
	state select_best_move(const board& b) const {
		...
		state* best = after;
		for (state* move = after; move != after + 4; move++) { //4次 上下左右
			if (move->assign(b)) {
				// TODO
				float value_sum = 0.f;
				int num = 0;
				board after_state_board = move->after_state();// 移動後的狀態，未加隨機值2 or 4
				for (int i = 0; i < 16; i++) {//在所有空tile上放上2或4並計算出board的期望值並且乘上加權
					if (after_state_board.at(i) == 0) 
					{
						after_state_board.set(i, 1);// 90% popup 2
						value_sum += 0.9 * estimate(after_state_board);
						after_state_board.set(i, 2);// 10% popup 4
						value_sum += 0.1 * estimate(after_state_board);
						after_state_board.set(i, 0);
						num++;
					}
				}
				move->set_value(move->reward() + value_sum / num);//設為before state的期望值
	 ...
	 */
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {//ok
		// TODO
		float yt = 0;
		path.pop_back();
		while (path.size())
		{
			...
			float error = yt  + move.reward() - estimate(move.before_state());//"error=V(S'')+r-V(S) 
			yt= update(move.before_state(), alpha * error);//更新參數 formula：yt=更新後的V(S)
			path.pop_back();
		}
	}
