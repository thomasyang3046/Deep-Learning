...
class Diffusion:
    ...
    def sample(self, model, n, labels, cfg_scale=4.1):
...
def train(args):
    ...
    dataloader=iclevrDataset(mode='train', root='dataset')
    dataloader = DataLoader(dataloader, batch_size=args.batch_size, shuffle=True)
  
    evaldata=iclevrDataset(mode= 'test', root='dataset')
    a=evaldata[0]
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % 10 == 0:
            labels = a.to(torch.float32).to(device)
